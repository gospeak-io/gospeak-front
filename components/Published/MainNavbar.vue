<template>
  <header
    class="bg-gpPurpleDark-900 sm:flex sm:justify-between sm:items-center sm:px-4 sm:py-3"
  >
    <div class="flex items-center justify-between px-4 py-3 sm:p-0">
      <div>
        <img class="h-8" src="~/static/gospeak-pink-light.svg" alt="Gospeak" />
      </div>
      <div class="sm:hidden">
        <button
          @click="isOpen = !isOpen"
    ...
    <nav :class="isOpen ? 'block' : 'hidden'" class="sm:block">
      <div class="px-2 pt-2 pb-4 sm:flex sm:p-0">
        ...
        <nuxt-link
          :to="{ name: 'speakers-slug', params: { slug: 'alberic-trancart' } }"
          class="mt-1 block px-2 py-1 text-white font-semibold rounded hover:bg-gray-800 sm:mt-0 sm:ml-2"
        >
          Groups
        </nuxt-link>
